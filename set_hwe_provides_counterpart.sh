...
For every -hwe package control file under <root-dir>, ensure Provides includes
its base counterpart package name.

Example:
  Package: qemu-utils-hwe
  Provides: ..., qemu-utils
...
for control_file in sorted(root.glob("*-hwe/*/control/control")):
    ...
    def rewrite_provides(match):
        ...
        merged = merge_item(normalized, counterpart)
    ...
    if not found_provides[0]:
        ...
        if match:
            start = match.start()
            rewritten = rewritten[:start] + f"Provides: {counterpart}\n" + rewritten[start:]
        else:
            if not rewritten.endswith("\n"):
                rewritten += "\n"
            rewritten += f"Provides: {counterpart}\n"

    if rewritten != original:
        updated += 1
        if dry_run:
            print(f"Would update: {control_file} -> Provides: {counterpart}")
        else:
            control_file.write_text(rewritten, encoding="utf-8")
            print(f"Updated: {control_file} -> Provides: {counterpart}")
