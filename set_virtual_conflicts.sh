...
Updates extracted Debian control files to ensure:
  - base packages conflict with: virt-hwe
  - -hwe packages conflict with: virt
...
field_pattern = re.compile(r"(?m)^(Conflicts):([^\n]*(?:\n[ \t].*)*)")
...
for control_file in sorted(root.glob("*/*/control/control")):
    scanned += 1
    package_name = control_file.parts[-4]
    conflict_item = "virt" if package_name.endswith("-hwe") else "virt-hwe"

    original = control_file.read_text(encoding="utf-8")

    found_conflicts = [False]

    def rewrite_conflicts(match):
        found_conflicts[0] = True
        ...
        merged = merge_item(normalized, conflict_item)
        return f"{field_name}: {merged}"

    rewritten = field_pattern.sub(rewrite_conflicts, original)

    if not found_conflicts[0]:
        insert_re = re.compile(r"(?m)^Section:[^\n]*$")
        match = insert_re.search(rewritten)
        if match:
            start = match.start()
            rewritten = rewritten[:start] + f"Conflicts: {conflict_item}\n" + rewritten[start:]
        else:
            if not rewritten.endswith("\n"):
                rewritten += "\n"
            rewritten += f"Conflicts: {conflict_item}\n"

    if rewritten != original:
        updated += 1
        if dry_run:
            print(f"Would update: {control_file} -> Conflicts: {conflict_item}")
        else:
            control_file.write_text(rewritten, encoding="utf-8")
            print(f"Updated: {control_file} -> Conflicts: {conflict_item}")
