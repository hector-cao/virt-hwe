...
if [ "$UNPACK" -eq 1 ]; then
  if ! command -v ar >/dev/null 2>&1; then
    echo "Error: ar is required for extraction." >&2
    exit 1
  fi

  if ! command -v tar >/dev/null 2>&1; then
    echo "Error: tar is required for unpacking control.tar.zst." >&2
    exit 1
  fi

  if ! command -v zstd >/dev/null 2>&1; then
    echo "Error: zstd is required for unpacking control.tar.zst." >&2
    exit 1
  fi
fi
...
if [ "$PACK" -eq 1 ]; then
  if ! command -v ar >/dev/null 2>&1; then
    echo "Error: ar is required for packaging." >&2
    exit 1
  fi

  if ! command -v tar >/dev/null 2>&1; then
    echo "Error: tar is required for creating control.tar.zst." >&2
    exit 1
  fi

  if ! command -v zstd >/dev/null 2>&1; then
    echo "Error: zstd is required for creating control.tar.zst." >&2
    exit 1
  fi
fi
...
extract_deb() {
  ...
  for member in control.tar.zst; do
    if printf '%s\n' "$members" | grep -Fxq "$member"; then
      ar p "$deb_path" "$member" > "$target_dir/$member"
      mkdir -p "$control_dir"
      zstd -dc "$target_dir/$member" | tar -x -f - -C "$control_dir"
      rm -f "$target_dir/$member"
    else
      echo "Missing member in $(basename "$deb_path"): $member"
    fi
  done
  ...
  echo "Removed intermediate archive: $target_dir/control.tar.zst"
